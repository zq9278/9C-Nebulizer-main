...
{
	app_ctx.lock = xSemaphoreCreateMutexStatic(&app_ctx.lock_storage);
	configASSERT(app_ctx.lock != NULL);
	...
	app_ctx.heartbeat_deadline_ms = runtime_now_ms() + APP_HOST_HEARTBEAT_TIMEOUT_MS;
	return 0;
}
...
{
	xSemaphoreTake(app_ctx.lock, portMAX_DELAY);
	*status = app_ctx.status;
	xSemaphoreGive(app_ctx.lock);
}
...
/* 只要已有故障码就允许清除，兼容故障事件尚未完成状态迁移的短暂窗口。 */
bool app_context_clear_fault(void)
{
	bool cleared;

	xSemaphoreTake(app_ctx.lock, portMAX_DELAY);
	cleared = (app_ctx.status.fault != FAULT_NONE);
	if (cleared) {
