#ifndef UART_PORT_H
#define UART_PORT_H
#include <platform/runtime.h>
#include <platform/hardware.h>
#include "uart_rx_ring.h"
typedef void (*uart_port_rx_notify_t)(void *user_data);
struct uart_port {
    const struct board_device *dev;
    struct uart_rx_ring rx_ring;
    StaticSemaphore_t rx_storage, tx_storage, lock_storage;
    SemaphoreHandle_t rx_sem, tx_done, tx_lock;
    uart_port_rx_notify_t notify;
    void *notify_user_data;
    const uint8_t *tx_buf;
    size_t tx_len, tx_pos;
    volatile uint32_t rx_errors, rx_overflows;
};
int uart_port_init(struct uart_port *port, const struct board_device *dev,
    uint8_t *rx_storage, size_t rx_storage_size, uart_port_rx_notify_t notify, void *notify_user_data);
int uart_port_send(struct uart_port *port, const uint8_t *data, size_t len, TickType_t timeout);
size_t uart_port_read(struct uart_port *port, uint8_t *data, size_t len);
int uart_port_wait_rx(struct uart_port *port, TickType_t timeout);
#endif
