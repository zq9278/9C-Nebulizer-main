...
- 预热阶段：雾化关闭，风扇固定低档运行（30%），不受治疗风量和雾量设置影响。达到 30.0°C 进入正式治疗后，再按所选档位启动风扇和雾化。开盖、暂停和故障联锁优先停机，不修改保存的档位设置。
- 磁铁盖采用 1000 ms 稳定消抖。治疗或预热中检测到原始开盖信号时立即关闭加热、风扇和雾化；开盖确认后进入暂停，重新盖好不会自动继续，必须由上位机发送恢复命令。
- 缺水时暂停并保留剩余治疗时间。加水后，等待雾化板报告水位恢复、解除安全锁定，盖子稳定闭合、上位机心跳正常且无故障，再自动继续剩余治疗；低于预热阈值时先预热。恢复请求持续重试，直到状态机确认恢复，不重新发送 START 或重置总时长。
